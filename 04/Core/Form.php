...
class Form
{
    ...
    public function render(string $action, string $elementName, array $user = []): string
    {
        $htmlForm = "<form method='POST' action='/" . $elementName . "' >";
        if ($action === 'update') {
    ...
    public function addError(string $name, string $errMessage): void
    {
        $this->errors[] = [
                'name' => $name,
                'err' => $errMessage
